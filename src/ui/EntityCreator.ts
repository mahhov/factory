...
export default class EntityCreator {
	...
	private static createToolTurret(metadata: ParsedLine<typeof sectionFields.turrets>) {
		...
		entity.addAttribute(materialStorageAttribute);
		let range = 16, speed = .4;
		let findTargetAttribute = new EntityFindTargetAttribute(range, 3, true, false);
		...
		entity.addAttribute(new EntityIfElseAttribute(
			...
			new EntityChainAttribute([
				...
				new EntitySpawnProjectileAttribute(findTargetAttribute, 3, 0, speed, range / speed, .2, 10, 10, true),
	...
	private static createPortalMob(metadata: ParsedLine<typeof sectionFields.mobs>) {
		return new PortalMob(metadata.size, metadata.health, 400, 1600, .05, .05);
	}
}
